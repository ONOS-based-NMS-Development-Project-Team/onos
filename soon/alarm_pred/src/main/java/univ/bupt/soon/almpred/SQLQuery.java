...
public class SQLQuery {
     ...
     * @return 返回查询结果
     */
    List<Item> queryData(String items, String constraint) {
     ...
     * @return 返回查询结果
     */
    List<Item> queryData(List<String> items, String constraint) {
        StringBuilder builder = new StringBuilder("SELECT ");
        for (String item : items) {
            builder.append(item).append(',');
        }
        builder.deleteCharAt(builder.length()-1);
        builder.append(" FROM alarm_prediction ").append(constraint);
        String query = builder.toString();
        log.info(query);
        // 开始查询ResultSet
        try {
            ResultSet rs = stmt.executeQuery(query);
            return parseResultSet(rs, AlarmPredictionItem.class);
